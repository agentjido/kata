...
## Keep a calibration case

Pick two or three findings in your own repo whose answers you are certain of,
write them down here, and re-check them after every change to the analyzer. A
good set covers both directions:

- a module you **know** is dead, with its test files, and what should happen to
  each (delete outright vs. surgical edit)
- a module you **know** is live but that is reached only dynamically -- through
  `quote`, a behaviour, a telemetry handler, config
- a protocol, if the project has one, which must stay **off** the
  unused-public-API list

If a change to the analyzer breaks any of them, the change is wrong. The classes
above were each found by exactly this: a result that contradicted something
already known to be true.

The project this skill came from used a superseded chunk writer (dead, with one
sibling test needing a surgical edit), its supervision-tree root reached only
through a `__using__` macro (live), and a protocol module for `defprotocol`
ownership.
